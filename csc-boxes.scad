...
function length(numUnits) = numUnits * unit_size + (numUnits - 1) * div_adjust;
...
module create_fitting_test() {
    ...
    // Setup one box in the origin location, mask out everything but the cutout
    intersection() {
        box(1, 1, total_floor_height, 1);
        box_mask_connector();
    }
    
    // Add a connector bar with a little distance
    ...
    translate([0, 0, connector_height / 2])
        union() {
            ...
            if (bar_width > 0) {
                ...
                cyl_len = length(length) - bar_overlap * 2;
            ...
            for (i = [1:length] ) {
               ...
               translate([conn_pos(i, length), -inner_radius + connector_overlap / 2, 0]) 
...
module box_mask_connector() {
    translate([0, outer_radius,total_floor_height / 2])
        scale([outer_radius * 2, outer_radius * 2, total_floor_height]) cube(1, true);
}
